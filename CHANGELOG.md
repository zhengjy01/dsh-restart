...
- **等待文案明确「请勿手动刷新」**：重启窗口内自己按刷新会落进浏览器的
  ...
  「端口会先空约 3–5 秒，请勿手动刷新：本页会自己回来」。（实测依据：18:03 那次重启 t+1.5s 新进程已在跑、t+4s 插件树已在处理请求——页面只要插件路由一应答就立刻自己刷新，**不等助手那个含 4s 确认窗的 `ready after 8.4s` 判定**；原先写 8–10 秒是把助手的判定账本当成了页面回来的时间。）
